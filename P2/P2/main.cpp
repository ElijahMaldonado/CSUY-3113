...
void Update() {
    ...
    if (playerOne_position.y < 3.02 && playerOne_position.y > -3.0) {
        leftMatrix = glm::mat4(1.0f);
        leftMatrix = glm::translate(leftMatrix, playerOne_position);
    }
    
    if (playerTwo_position.y < 3.02 && playerTwo_position.y > -3.0) {
        rightMatrix = glm::mat4(1.0f);
        rightMatrix = glm::translate(rightMatrix, playerTwo_position);
    }


    if (ball_position.y < 3 && ball_position.y > -3 && ball_position.x > -5 && ball_position.x < 5) {
        ball_position += ball_movement * ball_speed * deltaTime;
        ballMatrix = glm::mat4(1.0f);
        ballMatrix = glm::translate(ballMatrix, ball_position);

    } else if(ball_position.y < 0 || ball_position.y > 0) {
        ballMatrix = glm::mat4(1.0f);
        ball_speed = -ball_speed;
        ball_position += ball_movement * ball_speed * deltaTime;
        ballMatrix = glm::translate(ballMatrix, ball_position);
        
    } else if (ball_movement.x < 0 || ball_movement.x > 0) {
        ballMatrix = glm::mat4(1.0f);
        ball_speed = -ball_speed;
        ball_position += ball_movement * ball_speed * deltaTime;
        ballMatrix = glm::translate(ballMatrix, ball_position);
...
void Render() {
    ...
    float paddleVertices[] = {-0.5, -0.75, 0.5, -0.75, 0.5, 0.75, -0.5, -0.75, 0.5, 0.75, -0.5, 0.75};
