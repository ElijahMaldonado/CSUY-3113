...
Scene *currentScene;
Scene *sceneList[4]; //array of pointers to scenes
int sceneNum = 0;
...
void Initialize() {
    ...
    music = Mix_LoadMUS("computerNoise_002.mp3");
...
void Update() {    
    ...
    if (currentScene->state.player->collidedBottom && currentScene->state.player->collidedWithEnemyTop) {
    ...
    if (currentScene->state.player->lives == 0) {
        ...
        currentScene->state.enemies->jump = true;
    }
...
void Render() {
    ...
    if (currentScene->state.player->fail) {
        Util::DrawText(&program, fontTextureID, "FAIL!", 1, -0.5f, glm::vec3(6, -4, 0));
    }

    if (currentScene->state.player->win) {
        Util::DrawText(&program, fontTextureID, "WIN!", 1, -0.5f, glm::vec3(15, -3, 0));
    }
