...
void Initialize() {
    ...
    for (int i = 0; i < 18; i++) {
        state.platform[i+19].entityType = PLATFORM;
        state.platform[i+19].textureID = platformTextureID;
        state.platform[i+19].position = glm::vec3(-10, -7.5 + i, 0);
    }

    for (int i = 0; i < 18; i++) {
        state.platform[i + 35].entityType = PLATFORM;
        state.platform[i + 35].textureID = platformTextureID;
        state.platform[i + 35].position = glm::vec3(10, -7.5 + i, 0);
    }

    state.platform[53].textureID = platformTextureID;
    state.platform[53].position = glm::vec3(-9, -0.25, 0);

    state.platform[54].textureID = platformTextureID;
    state.platform[54].position = glm::vec3(-1, -2.25, 0);

    state.platform[55].textureID = platformTextureID;
    state.platform[55].position = glm::vec3(-3, -0.25, 0);

    state.platform[56].textureID = platformTextureID;
    state.platform[56].position = glm::vec3(-8, -0.25, 0);

    state.platform[57].textureID = platformTextureID;
    state.platform[57].position = glm::vec3(-7, -0.25, 0);

    state.platform[58].textureID = platformTextureID;
    state.platform[58].position = glm::vec3(-6, -0.25, 0);

    state.platform[59].textureID = platformTextureID;
    state.platform[59].position = glm::vec3(-5, -0.25, 0);

    state.platform[60].textureID = platformTextureID;
    state.platform[60].position = glm::vec3(-5, 0.75, 0);
...
void Update() {    
    ...
    while (deltaTime >= FIXED_TIMESTEP) {        
        ...
        for (int i = 0; i < ENEMY_COUNT; i++) {
            state.enemies[i].Update(FIXED_TIMESTEP,state.player, state.platform, PLATFORM_COUNT);
        }
